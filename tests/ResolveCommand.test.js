  ...
  describe("resolve", () => {
    ...
    describe("color option", () => {
      ...
      it("classifies variable references as 'variable'", async() => {
        ...
        // editor.background = $(bg), so trail should start with a variable
        const data = await command.resolve(theme, {color: "editor.background"})
        ...
        for(const step of varSteps) {
          assert.ok(step.value.startsWith("$"), `variable step "${step.value}" should start with $`)
        }
      })
      ...
      it("classifies function calls as 'expression'", async() => {
        ...
        // bg = lighten($(primary), 20), so trail should have an expression
        const data = await command.resolve(theme, {color: "editor.background"})
        ...
        for(const step of exprSteps) {
          assert.match(step.value, /\w+\(/, `expression step "${step.value}" should be a function call`)
        }
      })

      it("classifies final hex values as 'resolved'", async() => {
        const cwd = new DirectoryObject(__dirname)
        const packageJson = {}
        const command = new ResolveCommand({cwd, packageJson})
        command.setCache(new Cache())
      ...
      it("classifies direct hex values as 'literal'", async() => {
        const cwd = new DirectoryObject(__dirname)
        const packageJson = {}
        const command = new ResolveCommand({cwd, packageJson})
        command.setCache(new Cache())
        ...
        // editor.background = $(background), background = "#1a1a1a"
        // The trail should include a literal step for the direct hex
        const data = await command.resolve(theme, {color: "editor.background"})
        const literalSteps = data.trail.filter(s => s.type === "literal")

        // The direct hex "#1a1a1a" should be classified as literal
        if(literalSteps.length > 0) {
          for(const step of literalSteps) {
            assert.ok(
              !step.value.startsWith("$") && !/\w+\(/.test(step.value),
              `literal step "${step.value}" should not be a variable or function call`
            )
          }
        }
      })
