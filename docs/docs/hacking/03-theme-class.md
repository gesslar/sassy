...
Parses the source file. Format is auto-detected from the file extension (JSON5 or YAML). Populates `source` with the parsed content.
...
Writes the compiled output as `.color-theme.json`. Uses sha256 hashing to skip writes when the output has not changed. Supports dry-run mode via options.
